...
def parse_run(path):
    ...
    - 脚本模式：LLM 转述脚本 JSON 结果，实测两种形态——
      ...
      另含摘要行 **📌 每日热榜精选 - 日期**（09-18）。冒号在加粗号内部，
      标签正则在冒号后再吞一个可选 **；标题优先取带标签行，整行加粗
      兜底需排除摘要行，否则标题/URL 会被解析成 **。
    """
    ...
    # 脚本模式 g)：单行管道格式（09-05 实测）：**平台** | [标题](URL)
    # 无 🔗 行，平台与链接同行以 " | " 分隔
    g = re.search(r"^\*\*(.+?)\*\*\s*\|\s*\[(.+?)\]\((\S+?)\)\s*$", resp, re.M)
    if g:
        u = g.group(3)
        if not u.startswith("http"):
            return None
        plat = re.sub(r"\s*热榜$", "", g.group(1).strip()) or "未知"
        return {
            **base,
